...
interface CoinflipStatCardProps {
  label: string;
  value: string;
  hint?: string;
  loading?: boolean;
...
export function CoinflipStatCard({
  label,
  value,
  hint,
  loading,
  icon,
}: CoinflipStatCardProps) {
  if (loading) {
    return (
      <div
        className={cn(
  ...
  return (
    <div
      className={cn(
        ...
        "hover:-translate-y-0.5 hover:border-sky-500/25 hover:shadow-lg hover:shadow-sky-500/10"
      )}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {icon ? (
          <span className="text-zinc-600 transition-colors group-hover:text-sky-400/90">
            {icon}
          </span>
        ) : null}
      </div>
